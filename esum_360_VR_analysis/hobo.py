...
if __name__ == '__main__':
    ...
    #wd=getcwd()
    #chdir(wd)
    path = "C:\\Users\\vojha\Documents\ETH_project_data\VR_Data\Work\Weimar_360\processed\Hobo"
    hobo_data =  os.path.join(path,"Hobo_05.hobo")

    #if len(sys.argv) < 2:
    #    print('usage: %s CSVFILE' % sys.argv[0], file=sys.stderr)
        #sys.exit(2)
    #with HoboCSVReader(sys.argv[1]) as reader:
    with HoboCSVReader(hobo_data) as reader:
        for row in reader:
            print(row)