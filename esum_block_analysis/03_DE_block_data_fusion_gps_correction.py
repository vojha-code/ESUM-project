...
import math
#%% input date-time output timestamp
def make_timestamnp(timedata):
    return mktime(timedata.timetuple()) # timestamp float or int value
    
# input timestap number output date-time formate
def convert_from_timestamp(timestampNum):
    date_as_date_time = datetime.datetime.fromtimestamp(timestampNum)
    date_as_date_time = date_as_date_time + timedelta(hours=+int(0)) # correction for UTC time difference
    return date_as_date_time 

# input string date  output date-time formate
def convert_to_timestamp(date_in_str_other_format):
    date_as_date_time = dateutil.parser.parse(date_in_str_other_format)
    return date_as_date_time

# input string date  output date-time formate
def convert_to_timestamp_correction(deltaTime,date_in_str_other_format):
    date_as_date_time = dateutil.parser.parse(date_in_str_other_format)
    date_as_date_time = date_as_date_time + timedelta(hours=+int(deltaTime)) # correction for UTC time difference
    return date_as_date_time 
#%%
file_gps = os.path.join('C:\\Users\\vojha\Documents\ETH_project_data\VR_Data\Work\Weimar_Block\processed','Weimar_BlockGPS.csv')
data_gps = pd.read_csv(file_gps)

EDA_File_Raw = os.path.join('C:\\Users\\vojha\Documents\ETH_project_data\VR_Data\Work\Weimar_Block\processed\markedEDA',"eda_marked")
EDA_File_Pro = os.path.join("C:\\Users\\vojha\Documents\ETH_project_data\VR_Data\Work\Weimar_Block\processed\processedEDA","0p1")

VideoQ = os.path.join("C:\\Users\\vojha\Documents\ETH_project_data\VR_Data\Work\Weimar_Block\processed\Video","Q")
VideoT = os.path.join("C:\\Users\\vojha\Documents\ETH_project_data\VR_Data\Work\Weimar_Block\processed\Video","T")
fileSaves = os.path.join('C:\\Users\\vojha\Documents\ETH_project_data\VR_Data\Work\Weimar_Block\processed','compliedData')
file_complied = os.path.join('C:\\Users\\vojha\Documents\ETH_project_data\VR_Data\Work\Weimar_Block\processed','merged_data.csv')
file_groupXY = os.path.join('C:\\Users\\vojha\Documents\ETH_project_data\VR_Data\Work\Weimar_Block\processed','XYarusal.csv')
##
#%%
...
if(len(files_EDA) == len(files_LEDA) == len(files_Q) == len(files_T)):    
    ...
    for idx in range(len(files_EDA)):     
        ...
        if(files_EDA[idx][19:21] == files_LEDA[idx][9:11] == files_Q[idx][3:5] == files_T[idx][3:5]):
            ...
            VTPlay  = make_timestamnp(convert_to_timestamp(data_VT.iloc[0]["Date_Time"]))
            VtPause = make_timestamnp(convert_to_timestamp(data_VT.iloc[len(data_VT)-1]["Date_Time"]))
            ...
            for itrDataVT in range(len(data_VT)):
                if(data_VT["Trigger_Input"][itrDataVT] == "BREAK"):
                    print("",itrDataVT,data_VT["Trigger_Input"][itrDataVT])
                    if(itrDataVT < len(data_VT)-1):
                        VtPause = make_timestamnp(convert_to_timestamp(data_VT.iloc[itrDataVT-1]["Date_Time"]))
                        VTPlay  = make_timestamnp(convert_to_timestamp(data_VT.iloc[itrDataVT+1]["Date_Time"]))
            ...
            data_VT_5s = data_VT[data_VT["Trigger_Input"] != "BREAK"].reset_index(drop=True) # preserving VT
            data_VT_5s['Trigger_Input'] = data_VT_5s['Trigger_Input'].replace(['1'], '3')
            data_VT_5s['Trigger_Input'] = data_VT_5s['Trigger_Input'].replace(['0'], '2')
            data_VT_5s['Trigger_Input'] = data_VT_5s['Trigger_Input'].replace(['-1'], '1')
            data_VT_5s['Trigger_Input'] = data_VT_5s['Trigger_Input'].replace(['Null'], '0')

            data_VT_5s["Timestamp"] = data_VT_5s['Date_Time'].apply(convert_to_timestamp)
            data_VT_5s = data_VT_5s.set_index(pd.DatetimeIndex(data_VT_5s['Timestamp']))
            data_clicker = data_VT_5s[["Trigger_Input"]]            
            data_clicker["clicker_input"] = [int(i) for i in data_VT_5s.Trigger_Input.astype(float).tolist()]
            del data_clicker["Trigger_Input"]
            ...
            if(idx > 6 and idx < 20):
                time_eda_start = convert_to_timestamp_correction(deltaTime,data_eda_raw_5s.index[0].strftime('%Y-%d-%m %H:%M:%S.%f'))
                time_eda_end = convert_to_timestamp_correction(deltaTime,data_eda_raw_5s.index[len(data_eda_raw_5s)-1].strftime('%Y-%d-%m %H:%M:%S.%f'))
            else:
                time_eda_start = convert_to_timestamp_correction(deltaTime,data_eda_raw_5s.index[0].strftime('%Y-%m-%d %H:%M:%S.%f'))
                time_eda_end = convert_to_timestamp_correction(deltaTime,data_eda_raw_5s.index[len(data_eda_raw_5s)-1].strftime('%Y-%m-%d %H:%M:%S.%f'))
...
#            #Find Play time only
            ...
            while(itrEDA < len(data_leda)):
                if(idx > 6 and idx < 20):
                    time_eda = convert_to_timestamp_correction(deltaTime,data_eda_raw_5s.index[itrEDA].strftime('%Y-%dm-%m %H:%M:%S.%f'))
                else:
                    time_eda = convert_to_timestamp_correction(deltaTime,data_eda_raw_5s.index[itrEDA].strftime('%Y-%m-%d %H:%M:%S.%f'))
