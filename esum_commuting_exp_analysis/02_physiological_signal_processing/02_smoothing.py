...
#for i in [6,7,8,9,10,11,13,16,18,23,24,25,26,27,28,29,30,32,34,35]: #
    ...
    file_name = 'EDA_data_5s_marked.csv'
    #eda_file =  os.path.join(filepath,file_name)
    eda_data = pd.DataFrame.from_csv(file_name)
    ...
    refinedSC = butter_lowpass_filter(refinedSC, 1.0, 4, 4)
    
    #Original and filter signal
    fig, ax = plt.subplots(nrows=1, ncols=1, figsize=[5.5, 2])
    plt.plot(contaminatedSC, color="blue" , label="Original signal",linewidth = 0.5)
    plt.plot(refinedSC, color="red", label="Smoothed signal", linewidth = 0.5)
    ax.set_ylabel("$\mu S$")
    ax.set_xlabel("Time (ms)")
    plt.legend()
    plt.show()
