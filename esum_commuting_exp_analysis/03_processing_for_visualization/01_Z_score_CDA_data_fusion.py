# -*- coding: utf-8 -*-
"""
Created on Wed May 30 10:28:16 2018
...
for participant_int in CN:
    ...
    #% ledalab proces data
    path_ledalab_files = os.path.join(path_raw_data_files, "ledalab_batch_scrlist")
    #path_ledalab_files =  os.path.join(path_ledalab_files, "5seconds")
    nscr_file_name = "leda_exp_"+str(participant_int)+"_scrlist.xls"
    ...
    data_leda_manual["ZSCORE"] = stats.zscore(data_leda_manual["CDA.SCR-Amplitude"].tolist())
    data_leda_manual["ZSCORE_Peak"] = data_leda_manual["ZSCORE"].apply(lambda x: 1 if x > 0 else 0)
    data_leda_manual["ZSCORE_AMP"] = data_leda_manual["ZSCORE"].apply(lambda x: x if x > 0 else np.nan)
